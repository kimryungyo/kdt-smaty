...
- [ ] workspace: ABKO V4L2 → app FFmpeg → MediaMTX `/workspace-cam` → app RTSP receiver
...
- [ ] AKG Ara가 없는 오늘은 Voice가 기능별 `ERROR`로 격리되고 서버·Dashboard가 유지되는지
  검증한다.
- [ ] 내일 AKG Ara 연결 후 PortAudio의 정확한 input 이름을 설정하고 서버를 재기동한다.
  이번 단기 범위에는 무중단 hot-plug watcher를 추가하지 않는다.
- [ ] debug server는 production에서 끄거나 `127.0.0.1`에만 bind하고 transcript를 LAN에
  무인증 노출하지 않는다.
...
Voice model reasoning은 실시간 latency를 위해 현재 low를 유지한다. 구현 하위 세션에 사용하는
Terra/medium과 제품의 실시간 음성 reasoning 설정은 같은 요구가 아니다.
