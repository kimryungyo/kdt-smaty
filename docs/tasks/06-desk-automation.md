...
- [ ] task 05가 새 등록 또는 익명 session을 발행하면 그 session의 기본 control mode를 AUTO로 만든다.
- [ ] 등록 session은 profile 기본 작업 모드 snapshot과 LED를 적용하고, 익명 session은
...
- [ ] session 종료·교대에서 active mode와 LED override를 폐기하고 WLED OFF를 best-effort로
  요청한다. WLED 실패는 STOP·session 전이를 rollback하지 않는다.

### MANUAL, 작업 모드와 명령

- [ ] 직접 목표·HOLD의 MANUAL 전환과 기존 자동 generation 무효화를 직렬화하고 active 작업
  모드는 보존한다.
- [ ] 사용자 STOP과 안전 STOP의 control mode 결과를 구분하고 session 검증보다 먼저 처리한다.
- [ ] 같은 session의 명시적 AUTO 재활성화는 진행 이동 STOP → 후보 초기화 → AUTO 전환 →
  fresh 자세 5초 확인 순서로 처리한다.
- [ ] control/activity mode·Agents SDK Desk function tool의 `expectedSessionId`를 command lock 안에서
  비교한다.
- [ ] function tool은 model이 tool call을 만든 시점이 아니라 AutomationService command가
  실제 부작용을 실행하기 직전에 turn 시작 `sessionId`를 재검증한다.
- [ ] 작업 모드 선택 시 현재 profile의 default/custom 소유권과 값을 서버에서 다시 조회한다.
- [ ] AUTO에서 작업 모드를 바꾸면 control mode를 유지하고 이전 generation을 무효화한 뒤
  fresh·안정된 현재 자세로 새 높이를 평가한다. Vision이 불확실하면 mode·LED만 바꾸고 이동은 차단한다.
- [ ] MANUAL에서 작업 모드를 바꾸면 control mode를 유지하고 mode·LED만 바꾸며 책상을 움직이지 않는다.
- [ ] 수동 LED 변경은 session override로만 보관하고 다음 작업 모드·session에서 제거한다.
- [ ] task 03의 custom mode 삭제 API가 현재 active mode를 `409`로 거절하도록 active snapshot
  조회를 연결한다.
- [ ] session 없는 HOLD·직접 목표·STOP을 session이나 두 mode 생성 없이 처리한다.
- [ ] 장치 상태 때문에 수동 명령이 실패한 뒤 control/activity mode를 어떻게 유지할지 계약대로 적용한다.
- [ ] 기존 `/api/control`·`/api/target`을 AutomationService 경계로 이동한다.

### 상태·API·관측

- [ ] control mode 변경, activity mode 선택과 자동화 상태 API를 구현한다.
