...
async def test_write_commits_and_callback_error_rolls_back(tmp_path: Path) -> None:
    ...
    await database.write(
        lambda connection: connection.execute(
            ...
            ("profile-" + "1" * 32, "first", 80.0, 100.0, None, None, None),
        )
    )
...
async def test_version_two_data_migrates_once_to_profile_modes_without_loss(
    ...
    assert version == 5
    assert restored_profile == profile + (None, None)
...
async def test_profile_mode_foreign_key_cascades_on_profile_delete(tmp_path: Path) -> None:
    ...
    await database.write(
        lambda connection: (
            connection.execute(
                "INSERT INTO profiles VALUES (?, ?, ?, ?, ?, ?, ?)",
                (profile_id, "cascade", 80.0, 100.0, None, None, None),
            ),
            connection.execute(
...
async def test_version_four_data_migrates_to_version_five_with_nullable_new_columns(
    tmp_path: Path,
) -> None:
    path = tmp_path / "smart-desk.db"
