...
function SmatyDashboard() {
  ...
  // 머리말의 연결 표시는 높이 값이 신선한지까지 본다.
  const deskOnline = desk.value?.height.status === "ONLINE" && Boolean(desk.value?.relay.receivedAt) && !desk.value?.relay.lastError;
  // 명령을 받을 수 있는지는 릴레이만 보면 된다. 높이 값이 잠깐 끊겨도 서버는
  // 이동 직전에 센서를 다시 확인(WAKING)하므로, 그때문에 버튼을 막으면
  // 실제로는 멀쩡한데 조절이 안 되는 것처럼 보인다.
  const deskCommandable = Boolean(desk.value?.relay.receivedAt) && !desk.value?.relay.lastError;
