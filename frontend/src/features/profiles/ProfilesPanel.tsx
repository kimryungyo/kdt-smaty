...
export function HeightSetup({ profile, name, onSaved, onPrevious }: HeightsProps) {
  ...
  const complete = async () => {
    setSaving(true);
    try {
      const saved = profile ? await updateProfile(profile.id, { name, sittingHeightCm: sitting, standingHeightCm: standing }) : await createProfile({ name, sittingHeightCm: sitting, standingHeightCm: standing, ledColor: null });
      onSaved(saved);
